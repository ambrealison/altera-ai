...
# Phase 33J — French-first PT template.
# Headers are written in French so retailers can recognise the columns
# immediately; the mapping layer normalises them back to canonical
# snake_case (e.g. "Nom du produit" → product_name, "Poids unitaire
# produit" → weight_per_item_g). Identifier is optional — when blank,
# Altera generates an internal AUTO-… id.
_PT_HEADER = [
...
_PT_NOTES = [
    "# OPTIONNEL : identifiant produit / SKU dans votre système. Si vide, Altera génère un identifiant interne.",
    "# OBLIGATOIRE : nom du produit tel qu'il apparaît en rayon",
    "# recommandé : marque",
    "# recommandé : votre catégorie produit interne",
    "# recommandé : votre sous-catégorie produit interne",
    "# OBLIGATOIRE (au choix) : poids unitaire en kilogrammes (ex. 0.400 pour 400 g)",
    "# OBLIGATOIRE (au choix) : poids unitaire en grammes (ex. 400). Renseignez SOIT (kg), SOIT (g), pas les deux.",
    "# OBLIGATOIRE : nombre d'unités achetées sur la période de reporting",
    "# recommandé : protéines totales (% du poids du produit). Si vide, Altera peut enrichir depuis NEVO / CIQUAL.",
    "# optionnel : protéines végétales (% du poids du produit)",
    "# optionnel : protéines animales (% du poids du produit)",
    "# recommandé : true/false — produit de marque propre / MDD ?",
    "# recommandé : liste d'ingrédients lue sur l'étiquette",
    "# optionnel : labels séparés par | ex. bio|sans-gluten|vegan",
    "# optionnel : code pays ISO 3166-1 alpha-2, ex. FR",
    "# optionnel : code langue ISO 639-1, ex. fr",
    "# optionnel : code-barres EAN / GTIN",
    "# optionnel : période de reporting, ex. 2024-Q4",
]
...
_PT_EXAMPLE3 = [
    "",                      # Identifiant produit / SKU — laissé vide pour démontrer la génération auto
    "Salade de céréales mixtes",
    "AcmeChef",
    "Traiteur",
    "Salades & plats préparés",
    "",
    "250",                   # weight in grammes
    "530",
    "",                      # protéines totales manquantes — Altera enrichit
    "",
    "",
    "true",
    "Riz (30%), lentilles (25%), poivrons (20%), huile d'olive (10%), vinaigre (5%)",
    "",
    "FR",
    "fr",
    "",
